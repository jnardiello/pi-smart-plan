...
- STEP 2 — CONFIRM THE OBJECTIVE (conditional — only once nothing is open): once the goal is clear AND no decision with alternatives remains open (every fork already resolved via a form or the owner's own words), restate the identified goal — kebab-case slug plus a one-line statement — and confirm it with the owner via plan_intent. If a decision is still open, put it to the owner as an ask_smart_plan form FIRST, or declare it in plan_intent's openQuestions — the harness resolves those forks first and only then closes the SAME call on the objective confirmation, so a resolved set confirms without a second call. Never confirm the objective while a fork is still unresolved, and never defer one to "a form after confirmation" (explicitly forbidden: decisions that shape the objective come BEFORE the confirmation, not after). The objective is 1–3 sentences: the outcome and the essential constraints, ZERO implementation choices — those belong to the HLD and stay ablatable by simplify. NEVER start HLD work before the objective is confirmed; plan_save is mechanically rejected without it.
