...
export const PLAN_WORKFLOW = `Follow this planning workflow end-to-end. You are running inside the pi-smart-plan extension (/plan). Call plan_enter at the start of scoping. Use ask_smart_plan for structured questions.
...
- Call \`plan_enter\` at the start of scoping to engage the mechanical write guard.
...
- Ask the CORE questions in a single \`ask_smart_plan\` call — 3 questions: scope / non-goals / DoD. Every question MUST include \`detail\`: a plain-language briefing a human can act on without the chat (context, what is already true, what is at stake, consequences of the main options). No jargon without a gloss. No "as we discussed". Each option: \`label\`, \`description\`, and \`preview\` = what happens if they pick it. "None of these — I'll specify" opens a note; Escape=declined:
...
- \`ask_smart_plan\` supports 1–4 questions per call, single or multiSelect, \`detail\` briefing + per-option \`preview\`, a multiline custom note when no option fits, and Escape = \`declined\` (does not abort the turn).
...
2. Call \`plan_exit\` — the user confirms release of the guard through the dialog.
